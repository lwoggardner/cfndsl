  ...
  class Runner
    ...
    def self.invoke!
      ...
      optparse = OptionParser.new do |opts|
        ...
        options[:libraries] = []
        opts.on('-r', '--require', 'Require library') do |gem|
          options[:libraries] << gem
        end
      ...
      options[:libraries] << 'cfndsl' if options[:libraries].empty?
      options[:libraries].each { |l| require l }

      if options[:list]
        puts ObjectSpace.each_object(Class).select { |klass| klass < ResourceType }.map(&:resource_type).sort.join('\n')
        exit
      end
