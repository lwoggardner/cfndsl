...
module CfnDsl
  module AWS
    module Route53
      ...
      class RecordSetGroup < ResourceType
        # Property Definition RecordSet
        ...
        # @param [Array<RecordSet>|JSONable>] values
        #   The List of values to use for RecordSets
        def RecordSets(values)
          dsl_list_attribute(:RecordSets, values)
        end

        #  @param [RecordSet|JSONable] value  Append value to the List
        #    The List of values to use for RecordSets
        # @@overload RecordSet(&block)
        # Append value to the List from block via instance eval of a new RecordSet
        def RecordSet(value = nil, fn_if: nil, **value_hash, &block)
          dsl_push_attribute(:RecordSets, value, fn_if: fn_if, attr_class: RecordSet, **value_hash, &block)
        end
      end
    end
  end
  ...
  class CloudFormationTemplate
    ...
    def AWS_Route53_RecordSetGroup(logical_id, value = nil, &block)
      Resource(logical_id, value, attr_class: AWS::Route53::RecordSetGroup, &block)
    end
  end
