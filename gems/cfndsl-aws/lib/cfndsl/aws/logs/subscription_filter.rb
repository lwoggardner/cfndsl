# frozen_string_literal: true

# WARNING ABOUT GENERATED CODE
#
# This file is generated by CfnDsl::Generate
#
# WARNING ABOUT GENERATED CODE
require 'cfndsl/cloudformation_template'
require 'cfndsl/types'

module CfnDsl
  module AWS
    module Logs
      # Resource Type definition for AWS::Logs::SubscriptionFilter
      class SubscriptionFilter < ResourceType
        #   @param [String|JSONable] value
        #     The value to use for Id
        def Id(value)
          dsl_attribute(:Id, value)
        end

        #   @param [String|JSONable] value
        #     The value to use for DestinationArn
        def DestinationArn(value)
          dsl_attribute(:DestinationArn, value)
        end

        #   @param [String|JSONable] value
        #     The value to use for FilterPattern
        def FilterPattern(value)
          dsl_attribute(:FilterPattern, value)
        end

        #   @param [String|JSONable] value
        #     The value to use for LogGroupName
        def LogGroupName(value)
          dsl_attribute(:LogGroupName, value)
        end

        #   @param [String|JSONable] value
        #     The value to use for RoleArn
        def RoleArn(value)
          dsl_attribute(:RoleArn, value)
        end
      end
    end
  end

  # rubocop:disable Style/Documentation
  class CloudFormationTemplate
    # @return [AWS::Logs::SubscriptionFilter]
    def AWS_Logs_SubscriptionFilter(logical_id, value = nil, &block)
      Resource(logical_id, value, attr_class: AWS::Logs::SubscriptionFilter, &block)
    end
  end

  # rubocop:enable Style/Documentation
end
